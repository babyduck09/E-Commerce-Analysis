...
--> The total revenue generated by Olist from 2016-09-04 21:15:19 to	2018-10-17 17:30:18
```sql
...
How olist revenue change over time
```sql
SELECT -- Revenue by Quarter
...
```sql
-- The average order value (AOV) on Olist
WITH total_r AS (
SELECT 
...
```sql
-- active seller will be a seller who added a new listing in the last 30 days 
SELECT 
...
(SELECT -- one order can have mutiple review, we chose the highest review for that order
    review_score,
    order_id,
...
```
<img src="code_snapshot\avg_review_product.png" width="350" /> 
